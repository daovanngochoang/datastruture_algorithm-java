...
//            System.out.println(array);
    ...
    public int partitionArray(LinkedList<Edge<E>> array, int lIndex , int hIndex){
        ...
        for (int leftCounter = lIndex; leftCounter < hIndex; leftCounter++){ // run leftCounter to array
            ...
            if (array.get(leftCounter).weight > pivot.weight ) { // if leftCounter found an item that > pivot ==> rightCounter run
                while (leftCounter<rightCounter) { // rightCounter run but make sure it not pass the leftCounter
                    if (array.get(rightCounter).weight < pivot.weight) { // if rightCounter found an item < pivot => swap them and break.
    ...
    public void QuickSortTraditionalMethodNonRecursion2(LinkedList<GNode<E>> array){
        ...
        */
        ...
        if (len >= 2) {
            ...
            while (lTop > -1 && hTop > -1) { // we will do all sub-partitions in the index until it done.
                ...
                pivotIndex = partitionArray2(array, start, end); // put into the partition function for handling partitions
...
//            System.out.println(array);
    ...
    public int partitionArray2(LinkedList<GNode<E>> array, int lIndex , int hIndex){

        GNode<E> pivot = array.get(hIndex);
        int rightCounter = hIndex;
        GNode<E> temp;
        int pivotPosition = hIndex;
